...
class ConfigTests(unittest.TestCase):
    ...
    def test_execution_settings_are_loaded(self) -> None:
    ...
    def test_execution_settings_are_required(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            self.write_config(directory)
            values = json.loads(Path(directory, "config.json").read_text(encoding="utf-8"))
            del values["max_command_output_chars"]
            Path(directory, "config.json").write_text(json.dumps(values), encoding="utf-8")
            with patch.object(config, "CONFIG_DIR", directory):
